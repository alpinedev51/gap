...
class SpectralAnalyzer:
    ...
    def __init__(self, spatial_shape: Tuple[int, int] = (32, 32), device: str = "cpu"):
        ...
        self.bin_counts = torch.bincount(self.r_int, minlength=self.max_radius).clamp(min=1)
    ...
    def compute_1d_psd(self, psd_2d: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            bin_sums = torch.bincount(self.r_int, weights=psd_flat, minlength=self.max_radius)
            radial_psd[b] = bin_sums / self.bin_counts

        return radial_psd


    def get_log_psd(self, psd_tensor: torch.Tensor, epsilon: float = 1e-8) -> torch.Tensor:
    ...
    def plot_2d_difference(self, images_base: torch.Tensor, images_adv: torch.Tensor, title: str = "2D PSD Difference (Adv - Base)"):
    ...
    def plot_1d(self, images_dict: dict[str, torch.Tensor], title: str = "1D Azimuthally Averaged PSD", zoom_high_freq=False):
        """
    ...
    def plot_1d_ratio(self, images_base: torch.Tensor, images_adv: torch.Tensor, label: str = "PGD / Clean", zoom_high_freq: bool = False):
        """
        ...
        plt.axhline(y=1.0, color='black', linestyle='--', alpha=0.6, label="Baseline (Clean)")
        plt.fill_between(frequencies, 1.0, ratio, where=(ratio > 1.0), color='red', alpha=0.1)
        
        plt.title(f"Spectral Power Ratio: {label}" + (" (High-Freq Zoom)" if zoom_high_freq else ""), fontsize=14)
    ...
    def print_spectral_stats(self, images_dict: dict[str, torch.Tensor]):
        ...
        print(f"{'Dataset':<10} | {'Low-Freq Power (dB)':<20} | {'High-Freq Power (dB)':<20}")
        print("-" * 55)
