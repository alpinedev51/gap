...
from gap.utils import get_default_device, logger
...
def get_data_loader(dataset_name, batch_size=32, train=True, download=True):
    ...
    logger.info(f"Pinned Memory: {use_pin_memory}")
...
def _load_cifar10(train=True, download=True):
    training_set_for_stats = datasets.CIFAR10(
        root="./data", train=True, download=download, transform=transforms.ToTensor()
    )

    logger.info("Calculating CIFAR10 stats...")
    ...
    logger.info(f"Computed Std: {std}")
    ...
    if train:
        ...
        return datasets.CIFAR10(
            root="./data", train=False, download=download, transform=final_transform
        )
